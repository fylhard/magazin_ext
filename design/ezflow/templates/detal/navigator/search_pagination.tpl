...
{let page_count=int( ceil( div( $item_count,$item_limit ) ) )
      current_page=min($:page_count,
                       int( ceil( div( first_set( $view_parameters, 0 ),
                                       $item_limit ) ) ) )
...
{set view_parameter_text=concat('?',$getString)}
...
<ul>
       ...
       {case match=0}
      <li><a href={concat($page_uri,$:item_previous|gt(0)|choose('',concat($:offset_text,$:item_previous)),$:view_parameter_text,$page_uri_suffix)|ezurl}><span class="text">&laquo;&nbsp;</span></a></li>
       {/case}
       {case match=1}
       {/case}
     {/switch}
...
{switch match=$:item_next|lt($item_count)}
      {case match=1}
        <li><a href={concat($page_uri,$:offset_text,$:item_next,$:view_parameter_text,$page_uri_suffix)|ezurl}><span class="text">&nbsp;&raquo;</span></a></li>
      {/case}
      {case}
      {/case}
    {/switch}
